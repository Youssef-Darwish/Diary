...
class InfiniteCycleManager implements OnNotifyDataSetChangedListener {
    ...
    public void processAttributeSet(final AttributeSet attributeSet) {
        ...
        final TypedArray typedArray = mContext.obtainStyledAttributes(
                attributeSet, R.styleable.HorizontalInfiniteCycleViewPager
        );
        try {
            setMinPageScaleOffset(
                    typedArray.getDimension( R.styleable.HorizontalInfiniteCycleViewPager_icvp_min_page_scale_offset,
                            DEFAULT_MIN_PAGE_SCALE_OFFSET
                    )
            );
            setCenterPageScaleOffset(
                    typedArray.getDimension(R.styleable.HorizontalInfiniteCycleViewPager_icvp_center_page_scale_offset,
                            DEFAULT_CENTER_PAGE_SCALE_OFFSET
                    )
            );
            setMinPageScale(
                    typedArray.getFloat(R.styleable.HorizontalInfiniteCycleViewPager_icvp_min_page_scale,
                            DEFAULT_MIN_SCALE
                    )
            );
            setMaxPageScale(
                    typedArray.getFloat(R.styleable.HorizontalInfiniteCycleViewPager_icvp_max_page_scale,
                            DEFAULT_MAX_SCALE
                    )
            );
            setMediumScaled(
                    typedArray.getBoolean( R.styleable.HorizontalInfiniteCycleViewPager_icvp_medium_scaled,
                            DEFAULT_IS_MEDIUM_SCALED
                    )
            );
            setScrollDuration(
                    typedArray.getInteger(R.styleable.HorizontalInfiniteCycleViewPager_icvp_scroll_duration,
                            DEFAULT_SCROLL_DURATION
                    )
            );
            setPageDuration(
                    typedArray.getInteger(R.styleable.HorizontalInfiniteCycleViewPager_icvp_page_duration,
                            DEFAULT_SCROLL_DURATION
                    )
            );
            ...
            try {
                final int interpolatorId = typedArray.getResourceId(R.styleable.HorizontalInfiniteCycleViewPager_icvp_interpolator, 0
                );
